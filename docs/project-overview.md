...
Apple GPU data from structured `/usr/sbin/ioreg` output without superuser privileges, writes a
local SQLite file, and makes no network calls at runtime. NVIDIA and AMD adapters remain planned.

### 1.3 Deprecated / Retired or Not-Yet-Enabled Features

- **Not yet enabled:** GPU temperature/power, NVIDIA/AMD GPU adapters, and per-process GPU
  attribution. Apple GPU usage and memory fields are enabled on macOS.
...
summaries, process summaries, and storage status. Schema version 5 adds collector capabilities but
...
| Dashboard chart inspection | Pointer hover or chart focus | Show the nearest system bucket's average/min/max and overlay at most three bounded CPU and memory process series per dimension; memory values use the retained system-total sample for percentage conversion |
...
local `/usr/sbin/ioreg` and parses its property-list output. It deliberately avoids root-only
`powermetrics`; NVIDIA and AMD adapter choices are TBD — not yet designed.
...
metric or anomaly rows. Schema version 5 adds current collector capability state.
...
event checkpoint, and a 500-row per-event cap; executable paths are opt-in. `[gpu]` enables the
